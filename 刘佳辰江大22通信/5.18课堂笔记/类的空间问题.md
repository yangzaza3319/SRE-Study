...
- **内部添加**：在类的定义中直接定义的属性，属于类本身，所有对象共享。
...
# 访问类属性和对象属性
print(obj1.name)           # 输出: Object 1 访问对象属性
print(MyClass.class_attr)  # 输出: I am a class attribute 访问类属性
...
# 5. 类属性：外部添加
MyClass.new_class_attr = "I am a new class attribute" #外部动态添加类属性
...
mc2=MyClass("SRE")
#外部动态添加对象属性，各自独立，类的所有属性所有对象都共享
print(mc2.__dict__) #查看对象的所有属性
print(MyClass.__dict__) #查看类的所有属性
...
**对象查找属性的顺序**：
1. 先从对象空间找  -__init__()：pass

2. 类空间找  -MyClass

3. 父类空间找   -MyClass(OtherClass)
...
elphant = Elphant('小飞象') #大象类实例化
refrigerator = Refrigerator('格力冰箱') #冰箱类实例化
elphant.open(refrigerator) #依赖关系（需要在类的方法中传一个obj）
elphant.go()
...
boy2 = Boy('李四', girl) #在初始化时传递了另一个初始化的对象
boy2.dinner()
```
...
class Gamerole:
    ...
    def attack(self,obj):
        obj.hp -= self.ad
        print('%s攻击%s,%s掉了%s血,还剩%s'%(self.name,obj.name,obj.name,self.ad,obj.hp))

    def equip_weapon(self,wea_obj):
        self.wea = wea_obj #wea_obj 作为gamerole的属性
        wea_obj.ad += self.ad #gamerole ad属性赋予给wea_obj
        wea_obj.owner_name = self.name #gamerole name属性赋予给wea_obj
...
class Weapon:
    def __init__(self,name,ad,owner_name = None):
        ...
        self.ad = ad
    def weapon_attack(self,obj):
        obj.hp = obj.hp - self.ad
        print('%s利用%s攻击了%s，%s还剩%s血'%(self.owner_name,self.name,obj.name,obj.name,obj.hp))
